...
const SuperAdminSettings = () => {
    ...
    const handleToggle = (key) => {
        setSettings(prev => prev.map(s => {
            if (s.key === key) {
                return { ...s, value: s.value === '1' || s.value === 'true' ? 'false' : 'true' };
            }
            return s;
        }));
    };
                            ...
                            <div className="flex items-start justify-between p-4 bg-blue-50/50 rounded-xl border border-blue-100">
                                ...
                                <div className="flex items-center">
                                    <button
                                        ...
                                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${singleDomainLogin?.value === 'true' || singleDomainLogin?.value === '1' ? 'bg-blue-600' : 'bg-gray-300'
                                            }`}
                                    >
                                        <span
                                            className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${singleDomainLogin?.value === 'true' || singleDomainLogin?.value === '1' ? 'translate-x-6' : 'translate-x-1'
                                                }`}
                                        />
                                    </button>
                                </div>
