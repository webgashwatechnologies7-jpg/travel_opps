...
const LeadDetails = () => {
  ...
  const refreshProposalPricesFromServer = async () => {
    ...
    try {
      ...
      showToastNotification('error', 'Update Failed', 'Failed to load prices from server.');
    } finally {
    ...
    try {
      ...
      if (totalSent > 0) {
        ...
        if (itId && hotelsWithEmail.length > 0) {
          (async () => {
            try {
              const pkgRes = await packagesAPI.get(itId);
              ...
              if (changed) {
                await packagesAPI.update(itId, { day_events: updatedEvts });
                loadHotelsFromAllProposals();
              }
                                            ...
                                            >
                                              ...
                                              <div className="p-4">
                                                <div className="mb-4">
                                                  ...
                                                  {/* Hotels Summary */}
                                                  {(meta.hotelOptions || opt.hotelOptions || meta.hotels || opt.hotels) ? (
                                                    <div className="space-y-1 mb-4 max-h-32 overflow-y-auto pr-1 thin-scrollbar">
                                                      {(meta.hotelOptions || opt.hotelOptions || meta.hotels || opt.hotels || []).map((h, hIdx) => (
                                                    ...
                                                    <>
                                                      <button
                                                        ...
                                                        onClick={async (e) => {
                                                          ...
                                                          setHotelManagerData([]);
                                                          try {
                                                            const res = await packagesAPI.get(itId);
                                                            const pkg = res?.data?.data;
                                                            const dayEvts = pkg?.day_events || {};
                                                            const startDate = pkg?.start_date || lead?.travel_start_date || null;
                                                            const leadEnd = lead?.travel_end_date || null;

                                                            const calcDate = (dayNum) => {
                                                              if (!startDate) return '';
                                                            ...
                                                            const isDateInRange = (dateStr) => {
                                                              if (!dateStr || !startDate || !leadEnd) return true;
                                                              return dateStr >= startDate && dateStr <= leadEnd;
                                                            };

                                                            const hotels = [];
                                                            Object.keys(dayEvts).sort((a, b) => parseInt(a) - parseInt(b)).forEach(dayNum => {
                                                              const evts = dayEvts[dayNum] || [];
                                                              evts.forEach((ev, evIdx) => {
                                                                if ((ev.eventType || '').toLowerCase() === 'accommodation' && ev.hotelOptions?.length) {
                                                                  ev.hotelOptions.forEach((hotelOpt, optIdx) => {
                                                                    const autoIn = calcDate(parseInt(dayNum));
                                                                    const autoOut = calcDate(parseInt(dayNum) + 1);

                                                                    // Prefer DB date ONLY if it's within lead range
                                                                    const finalIn = isDateInRange(hotelOpt.checkIn) ? hotelOpt.checkIn : autoIn;
                                                                    const finalOut = isDateInRange(hotelOpt.checkOut) ? hotelOpt.checkOut : autoOut;
                                                                    ...
                                                                    hotels.push({
                                                                      ...
                                                                      eventIndex: evIdx,
                                                                      optIndex: optIdx,
                                                                      hotelName: hotelOpt.hotelName || 'Hotel',
                                                                      category: hotelOpt.category || '',
                                                                      mealPlan: hotelOpt.mealPlan || '',
                                                                      roomName: hotelOpt.roomName || '',
                                                                      roomCount: hotelOpt.roomCount || hotelOpt.rooms || (ev.roomCount || 1),
                                                                      checkIn: finalIn || autoIn,
                                                                      checkOut: finalOut || autoOut,
                                                                      image: hotelOpt.image || null,
                                                                      status: hotelOpt.status || 'Pending',
                                                                    });
                                                                  });
                                                                }
                                                              });
                                                            ...
                                                            showToastNotification('error', 'Error', 'Failed to load itinerary data');
                                                            setShowHotelManagerModal(false);
                                                          } finally {
                                                            setLoadingHotelManager(false);
                                                          }
                                                        }}
                                                        className="w-full inline-flex items-center justify-center gap-1.5 text-blue-600 hover:text-blue-800 hover:bg-blue-50 text-sm font-semibold px-3 py-2 rounded-lg border border-blue-200 transition-colors"
                                                      >
                          ...
                          try {
                            ...
                            hotelManagerData.forEach(hotel => {
                              ...
                              if (optToUpdate) {
                                ...
                                optToUpdate.rooms = hotel.roomCount; // Keep both for safety
