  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      >
        ...
        {hero.tagline && (
          <p className="inline-block bg-amber-400 text-gray-900 px-4 py-1 rounded font-semibold">{hero.tagline}</p>
        )}
        {hero.formTitle && (
          <div className="mt-8 max-w-sm mx-auto bg-white/10 backdrop-blur rounded-lg p-6">
            ...
            <form className="space-y-3">
              ...
              <button type="submit" className="w-full bg-amber-400 text-gray-900 py-2 rounded font-semibold hover:bg-amber-500">
                SEND ENQUIRY
              </button>
            </form>
          </div>
      ...
      {about.title && (
        <section className="py-16 px-6 max-w-4xl mx-auto">
          ...
          {about.ctaText && about.ctaPhone && (
            <a
              href={`tel:${about.ctaPhone}`}
              className="inline-block bg-amber-400 text-gray-900 px-6 py-3 rounded font-semibold hover:bg-amber-500"
            >
              {about.ctaText}
            </a>
          )}
                ...
                <div className="p-4">
                  ...
                  {(pkg.inclusions || []).length > 0 && (
                    <ul className="mt-2 text-sm text-gray-600 space-y-1">
                      {pkg.inclusions.slice(0, 3).map((inc, j) => (
                        <li key={j}>• {inc}</li>
                      ))}
                    </ul>
                  )}
                  <p className="mt-2 font-bold text-teal-600">Starts from ₹{pkg.price}/-</p>
                  <button className="mt-3 w-full bg-amber-400 text-gray-900 py-2 rounded font-semibold hover:bg-amber-500">
                    SEND ENQUIRY
                  </button>
                </div>
              </div>
