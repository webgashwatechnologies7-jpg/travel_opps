...
const ExpenseType = () => {
  ...
  const [editingExpenseTypeId, setEditingExpenseTypeId] = useState(null);
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingExpenseTypeId) {
        await expenseTypesAPI.update(editingExpenseTypeId, formData);
        setError('');
      } else {
        await expenseTypesAPI.create(formData);
        setError('');
      }
      ...
      setError(err.response?.data?.message || (editingExpenseTypeId ? 'Failed to update expense type' : 'Failed to add expense type'));
      console.error(err);
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to delete expense type');
      console.error(err);
    }
  };
    ...
    <Layout>
      <div className="p-6" style={{ backgroundColor: '#D8DEF5', minHeight: '100vh' }}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Expense Type</h1>
                  ...
                  filteredExpenseTypes.map((expenseType) => (
                    <tr key={expenseType.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">{expenseType.name || 'N/A'}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${expenseType.status === 'active'
                            ? 'bg-green-100 text-green-800'
                            : 'bg-red-100 text-red-800'
                          }`}>
                          {expenseType.status === 'active' ? 'Active' : 'Inactive'}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{expenseType.created_by_name || 'Travbizz Travel IT Solutions'}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {formatDate(expenseType.updated_at || expenseType.last_update)}
                        </div>
                      </td>
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
              ...
              <div className="flex justify-between items-center p-6 border-b border-gray-200">
                <h2 className="text-xl font-bold text-gray-800">
                  {editingExpenseTypeId ? 'Edit Expense Type' : 'Add Expense Type'}
                </h2>
                <button
              ...
              <form onSubmit={handleSave}>
                ...
                <div className="flex justify-end gap-3 p-6 border-t border-gray-200">
                  ...
                  >
                    {saving ? (editingExpenseTypeId ? 'Updating...' : 'Saving...') : 'Save'}
                  </button>
                </div>
              </form>
            </div>
