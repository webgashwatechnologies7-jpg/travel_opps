  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingRoomTypeId) {
        await roomTypesAPI.update(editingRoomTypeId, formData);
        setError('');
      } else {
      ...
      setError(err.response?.data?.message || (editingRoomTypeId ? 'Failed to update room type' : 'Failed to add room type'));
      console.error(err);
    } finally {
      setSaving(false);
    }
                  ...
                  filteredRoomTypes.map((roomType) => (
                    <tr key={roomType.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">{roomType.name || 'N/A'}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${roomType.status === 'active'
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{roomType.created_by_name || 'Travbizz Travel IT Solutions'}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {formatDate(roomType.updated_at || roomType.last_update)}
                        </div>
                      </td>
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
              ...
              <div className="flex justify-between items-center p-6 border-b border-gray-200">
                <h2 className="text-xl font-bold text-gray-800">
                  {editingRoomTypeId ? 'Edit Room Type' : 'Add Room Type'}
                </h2>
                <button
              ...
              <form onSubmit={handleSave}>
                ...
                <div className="flex justify-end gap-3 p-6 border-t border-gray-200">
                  ...
                  >
                    {saving ? (editingRoomTypeId ? 'Updating...' : 'Saving...') : 'Save'}
                  </button>
                </div>
              </form>
            </div>
