...
const LeadSource = () => {
  ...
  const fetchLeadSources = async () => {
    try {
      ...
      setLeadSources(response.data.data || response.data || []);
      setError('');
    } catch (err) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingLeadSourceId) {
        await leadSourcesAPI.update(editingLeadSourceId, formData);
        setError('');
      } else {
      ...
      setError(err.response?.data?.message || (editingLeadSourceId ? 'Failed to update lead source' : 'Failed to add lead source'));
      console.error(err);
    } finally {
      setSaving(false);
    }
  ...
  const handleEdit = (leadSource) => {
    setEditingLeadSourceId(leadSource.id);
    setIsModalOpen(true);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to delete lead source');
      console.error(err);
    }
  };

  const filteredLeadSources = leadSources.filter(leadSource => {
    const name = leadSource.name || '';
    ...
    <Layout>
      <div className="p-6" style={{ backgroundColor: '#D8DEF5', minHeight: '100vh' }}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Lead Source</h1>
                  ...
                  filteredLeadSources.map((leadSource) => (
                    <tr key={leadSource.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">{leadSource.name || 'N/A'}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${leadSource.status === 'active'
                            ? 'bg-green-100 text-green-800'
                            : 'bg-red-100 text-red-800'
                          }`}>
                          {leadSource.status === 'active' ? 'Active' : 'Inactive'}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{leadSource.created_by_name || 'Travbizz Travel IT Solutions'}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          {formatDate(leadSource.updated_at || leadSource.last_update)}
                        </div>
                      </td>
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
              ...
              <div className="flex justify-between items-center p-6 border-b border-gray-200">
                <h2 className="text-xl font-bold text-gray-800">
                  {editingLeadSourceId ? 'Edit Lead Source' : 'Add Lead Source'}
                </h2>
                <button
              ...
              <form onSubmit={handleSave}>
                ...
                <div className="flex justify-end gap-3 p-6 border-t border-gray-200">
                  ...
                  >
                    {saving ? (editingLeadSourceId ? 'Updating...' : 'Saving...') : 'Save'}
                  </button>
                </div>
              </form>
            </div>
