...
class Company extends Model
{
    ...
    /**
     * Get CRM URL for the company (with crmopp prefix).
     *
     * @return string
     */
    ...
    {
        if ($this->domain) {
            // Extract the base domain from the domain field
            // If domain is already like test.com, use it directly
            // If it contains subdomains, extract the main domain
            $domainParts = explode('.', $this->domain);
            if (count($domainParts) >= 2) {
                // Take the last two parts as the main domain (e.g., test.com)
                $mainDomain = $domainParts[count($domainParts) - 2] . '.' . $domainParts[count($domainParts) - 1];
                return 'https://crmopp.' . $mainDomain;
            }
            return 'https://crmopp.' . $this->domain;
        }
        // For localhost or default domain
        return 'https://crmopp.' . $this->subdomain . '.' . config('app.domain', 'localhost');
    }
}
